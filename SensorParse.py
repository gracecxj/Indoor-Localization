...
num_grid_y = 60  # latitude
num_grid_x = 80  # longitude
max_lat = abs(north_west[0] - south_east[0])  # 0.0006
max_lng = abs(north_west[1] - south_east[1])  # 0.002
delta_lat = max_lat / num_grid_y  # 3e-06
delta_lng = max_lng / num_grid_x  # 1e-05

ROW_SIZE = 2000
NUM_COLUMNS = 3*100 + 3*100 + 1*92
...
# 但一个WiFi record包括92个值，这是由"wifi_id.txt"确定的）
...
# 返回值：传入的是acc/mag返回值为3个，传入的是wr时返回值为92个
...
class SensorFile(object):
    ...
    def __init__(self, file_name):
        ...
        self.f_outputs = np.zeros((self.sample_num, 3))
    ...
    def threshold_and_filter(self):
        ...
        for k, v in self.loc_dict.items():
            ...
            # add wr(600:692) into f_inputs
            self.f_inputs[k[0], 600:] = np.array(wr_reduced).reshape(1, 92)

            # add (x,y,index) into f_outputs
    ...
    # write the overall standard input and output into a single ".txt" file
    def save_overall_txt(self):
        txt_filename = "./background_results/out_in_overall.txt"
        write_text = np.hstack((self.f_outputs, self.f_inputs))
        with open(txt_filename, "ab") as f:
            np.savetxt(f, write_text, delimiter=",", newline='\n')

    # write the overall standard input and output into a single ".h5" file
    def save_overall_hdf5(self):
        h5_filename = "./background_results/out_in_overall.h5"
...
file1 = "./background_results/out_in_overall.h5"
file2 = "./background_results/out_in_overall.txt"
